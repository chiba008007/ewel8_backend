...
class ExamBaj4Controller extends Controller
{
    //

    public function getBAJ4(Request $request)
    {
        $request->validate([
        ...
        // 取得API到達ログ
        Log::info('BAJ4取得API到達', [
            'exam_id' => $examId,
            'testparts_id' => $testpartsId,
            'ip' => $request->ip(),
        ]);

        $last = ExamBaj4::where('exam_id', $examId)
    ...
    public function setBAJ4(Request $request)
    {
        $examId = auth()->id();
        $testpartsId = $request->testparts_id;

        Log::info('BAJ4開始API到達', [
            'exam_id' => $examId,
            'testparts_id' => $testpartsId,
            'ip' => $request->ip(),
        ]);
    ...
    public function editBAJ4(Request $request)
    {
        $exam_id = auth()->id();
        $testparts_id = $request->testparts_id;

        Log::info('BAJ4検査回答登録開始', [
            'exam_id' => $exam_id,
        ...
        try {
            DB::transaction(function () use ($request, $exam_id, $testparts_id) {
                // 最新の受検データを取得
                $exam = ExamBaj4::where('testparts_id', $testparts_id)
                ...
                if ((int) $request->page === 5) {
                    // BAJ4結果を計算・保存
                    $this->resultBaj4($testparts_id);

                    Log::info('BAJ4採点結果保存成功', [
                        'exam_id' => $exam_id,
                        'testparts_id' => $testparts_id,
                        'exambaj4_id' => $exam->id,
                    ]);
    ...
    public function resultBaj4($testparts_id)
    {
        ...
        $exam_id = $user->id;
        // 最後の1件を取得
        $last = ExamBaj4::select("*")->latest("id")->where("testparts_id", $testparts_id)->where("exam_id", $exam_id)->first();
        // 重みデータ取得
